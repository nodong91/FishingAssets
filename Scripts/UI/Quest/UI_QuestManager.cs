...
public class UI_QuestManager : MonoBehaviour
{
    ...
    public UI_QuestSlot[] questSlots;
    ...
    Queue<UI_QuestSlot> queueSlot = new Queue<UI_QuestSlot>();
    ...
    UI_QuestSlot TrySlot()
    {
        if (queueSlot.Count > 0)
            return queueSlot.Dequeue();
        UI_QuestSlot inst = Instantiate(questSlot, gridParent.transform);
        return inst;
    }
    ...
    public void SetQuest(Data_Quest[] _questData)
    {
        ...
        questSlots = new UI_QuestSlot[_questData.Length];
        for (int i = 0; i < _questData.Length; i++)
        {
            UI_QuestSlot slot = TrySlot();
    ...
    IEnumerator CloseCanvas()
    {
        ...
        while (normalize < 1f)
        {
            ...
            //questCanvas.transform.localScale = Vector3.one * (1f-alpha);
            yield return null;
        }
        questCanvas.gameObject.SetActive(false);
    }
